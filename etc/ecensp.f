        ...
        logical alive
        integer :: ncid,status,xtype,ndims,nvars,DimID,VarID,NX,NY,NN,NT
        integer :: VarNum,nGlobalAtts,unlimitedDimId,i0,j0,n0,k0,k1
        integer :: tp_FillValue,tp_missing_value,m,n
        character(LEN=50) :: xname,yname,tname,vname,argstr
        real,allocatable :: tpdata(:,:,:,:),tp(:,:,:,:),czh_tp(:,:,:,:)
        ...
        integer(KIND=2),allocatable :: npos(:),npos_out(:)
...
!C  get the dimensions names and number
     ...
     1    DimNs(DimID))
         ...
         allocate(npos(DimNs(3)))
         allocate(tpos(DimNs(4)))
         allocate(tpos_out(k1))
         allocate(tpdata(DimNs(1),DimNs(2),DimNs(3),DimNs(4)))
         allocate(tp(DimNs(1),DimNs(2),DimNs(3),DimNs(4)))
         allocate(tpdata6h(DimNs(1),DimNs(2),DimNs(3),k1))
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!    
!C get the variables names Types number and IDs
     ...
     1      VarIds(VarNum)))
        ...
        enddo    
C              call check(nf90_inquire_attribute(ncid,VarIds(VarNum),
C      1      "units",AttTypes(VarNum),AttLens(VarNum),
C      1      AttNums(VarNum)))
C              write(*,*)'AttType=',AttTypes(VarNum)
C              write(*,*)'Attlen=',AttLens(VarNum)
C              write(*,*)'AttNum=',AttNums(VarNum)
        
        call check(nf90_get_var(ncid,VarIds(1),xpos))
        CALL check(nf90_get_var(ncid,VarIds(2),ypos))
        CALL check(nf90_get_var(ncid,VarIds(3),npos))
        CALL check(nf90_get_var(ncid,VarIds(4),tpos))
        write(*,*)"npos=",npos
        write(*,*)tpos
        CALL check(nf90_get_att(ncid,VarIds(5),"scale_factor",
     1  tp_scale_factor))
        write(*,*)'tp_scale_factor =',tp_scale_factor
        CALL check(nf90_get_att(ncid,VarIds(5),"add_offset",
     1  tp_add_offset))
        write(*,*)'tp_add_offset =',tp_add_offset
        CALL check(nf90_get_att(ncid,VarIds(5),"_FillValue",
     1  tp_FillValue))
        write(*,*)'tp_FillValue =',tp_FillValue
        CALL check(nf90_get_att(ncid,VarIds(5),"missing_value",
     1  tp_missing_value))
        write(*,*)'tp_missing_value =',tp_missing_value
        CALL check(nf90_get_var(ncid,VarIds(5),tpdata))
        call check(nf90_close(ncid))
        write(*,*)'*** SUCCESS getting ncddata ***'
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!C get the grid preciption data
        do k0=1,DimNs(4)
            ...
            do j0=1,DimNs(2)
            do i0=1,DimNs(1)
                tp(i0,j0,n0,k0)=(tpdata(i0,j0,n0,k0)*tp_scale_factor
     1          +tp_add_offset)*1000
                if (tp(i0,j0,n0,k0) < 0) then
                    write(*,'(3i4,f8.1)')i0,j0,k0,tp(i0,j0,n0,k0)
                endif
            enddo
            enddo
            enddo
        enddo
        write(*,*)'*** SUCCESS getting rain data '
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!C get the 间隔 grid preciption data
        ...
        do j0=1,DimNs(2)
        do i0=1,DimNs(1)
            tpdata6h(i0,j0,n0,k0)=tp(i0,j0,n0,k0+1)-tp(i0,j0,n0,k0)
            if (tpdata6h(i0,j0,n0,k0) < -0.1) then
                write(*,'(4i4,f8.1)')i0,j0,n0,k0,tpdata6h(i0,j0,n0,k0)
            endif
        enddo
        enddo
        enddo
        enddo
        write(*,*)'*** SUCCESS getting betwin taime data '
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!C 给定范围 (120-100)/0.5=40,(40-25)/0.5=30,(100-40)/0.5=120,(70-40)/0.5=60
        ...
        allocate(m_tpdata6h(gdnf_x+1,gdnf_y+1,50,k1))
        ...
        enddo
        allocate(npos_out(50))
        do n0=1,50
         npos_out(n0)=npos(n0)
        enddo
        write(*,*)'npos=',npos
        write(*,*)'npos_out=',npos_out
        do k0=1,k1
        do n0=1,50
        do j0=1,gdnf_y+1
        do i0=1,gdnf_x+1
            m_tpdata6h(i0,j0,n0,k0)=tpdata6h(gdni_x+i0,gdni_y+j0,n0,k0)
            if (m_tpdata6h(i0,j0,n0,k0) < -0.1) then
                write(*,'(4i4,f8.1)')i0,j0,n0,k0,m_tpdata6h(i0,j0,n0,k0)
            endif
        enddo
        enddo
        enddo
        enddo
        write(*,*)'*** SUCCESS cutting data '
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!        
...
130     format(3f8.1) 
        allocate(czh_tp(gdnl_x+1,gdnl_y+1,50,k1))
        do k0=1,k1
        do n0=1,50
        do j0=1,gdnl_y+1
        do i0=1,gdnl_x+1
...
C      1      m_tpdata6h(m,n+1,n0,k0),m_tpdata6h(m+1,n+1,n0,k0)
            ...
            y1=1-(tn/ns)
            y2=1-(ts/ns)
         t1=(x1*m_tpdata6h(m,n,n0,k0))  +(x2*m_tpdata6h(m+1,n,n0,k0))
         t2=(x3*m_tpdata6h(m,n+1,n0,k0))+(x4*m_tpdata6h(m+1,n+1,n0,k0))
            czh_tp(i0,j0,n0,k0)=(y1*t1) +(y2*t2)
C             if (MOD(i0,4) == 1) then
C                 if (MOD(j0,4) == 1) then
C             write(*,'(2i3,10f6.3)')m,n,tw,te,tn,ts,x1,x2,x3,x4,y1,y2
C             write(*,130)m_tpdata6h(m,n,n0,k0),t1,
C      1       m_tpdata6h(m+1,n,n0,k0)
C             write(*,'(8x,f8.1)')czh_tp(i0,j0,n0,k0)
C             write(*,130)m_tpdata6h(m,n+1,n0,k0),t2,
C      1      m_tpdata6h(m+1,n+1,n0,k0)
C                 endif
C             endif
            if (czh_tp(i0,j0,n0,k0) < -0.1) then
                write(*,*)i0,j0,k0,czh_xpos(i0),czh_ypos(j0),
     1          czh_tpos(k0),czh_tp(i0,j0,n0,k0)
            endif
        enddo
        enddo
        enddo
...
200     format(a8,i1,1x,a10,a,a10,a,a10,a,a2,a)
        do k0=1,k1
     ...
     1      day_out,hour_out)
        ...
        call adddatetime(year_out,month_out,day_out,hour_out,-24,"hr")
            write(*,*)"prer_date=",year_out,month_out,day_out,hour_out
            write(pre_date,300)year_out,month_out,day_out,hour_out
            do n0=1,50   
                write(numberstr,'(i2.2)')npos_out(n0)
            write(comment,200)'diamond ',diamond_type,initial_date,
     1     "_",pre_date,"-",cur_date,"number",numberstr,"EC24Hrain"
                write(*,*)comment
                write(mhz,'(i3.3)')(k0*24+12)
                write(*,*)mhz
                Fname_3=trim(rawpath)//"/P_"//numberstr//"_"//
     1          pre_date//"-"//cur_date(5:10)//"_M3."//mhz
...
!C 生成micaps 3类 插值后数据
     ...
     1      day_out,hour_out)
            ...
            write(cur_date,300)year_out,month_out,day_out,hour_out
        call adddatetime(year_out,month_out,day_out,hour_out,-24,"hr")
            write(*,*)"pre_date=",year_out,month_out,day_out,hour_out
            write(pre_date,300)year_out,month_out,day_out,hour_out
            do n0=1,50   
                write(numberstr,'(i2.2)')npos_out(n0)
                write(comment,200)'diamond ',diamond_type,initial_date,
     1     "_",pre_date,"-",cur_date,"number",numberstr,"EC24Hrain"
                ...
                write(*,*)mhz
                Fname_3=trim(rawpath)//"/P_"//numberstr//"_"//
     1          pre_date//"-"//cur_date(5:10)//"_M3_INS."//mhz
                open(10,file=trim(Fname_3),iostat=status)
                write(10,*)comment(:len_trim(comment))
                write(10,150)year_out,month_out,day_out,hour_out,-2
                write(10,'(a)')'0'
                write(10,'(a)')'1 2.5 0 '
                write(10,'(i5,i6')1,(gdnl_x+1)*(gdnl_y+1)
                do j0=1,gdnl_y+1
                do i0=1,gdnl_x+1
                    write(10,140)i0*j0,czh_xpos(i0),czh_ypos(j0),1,
     1              czh_tp(i0,j0,n0,k0)
                enddo
                enddo
                close(10)
                print *, "SUCCESS writing insert m3 data /"//Fname_3
            enddo
        enddo        
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!C 生成micaps 4类 插值 前 数据 
        diamond_type = 4
300     format(i4,3i2.2)
...
550     format(10f8.1)
     ...
     1      day_out,hour_out)
        ...
        write(*,*)"pre_date=",year_out,month_out,day_out,hour_out
            ...
            write(numberstr,'(i2.2)')npos_out(n0)
            write(comment,200)'diamond ',diamond_type,initial_date,
     1     "ini",pre_date,"to",cur_date,"number",numberstr,"EC24HRain"
                write(*,*)comment
                write(mhz,'(i3.3)')(k0*24+12)
                write(*,*)mhz
                filename_4=trim(rawpath)//"/P_"//numberstr//"_"//
...
100     format(2f7.3,6i4,a)
        write(30,100)gd_f,gd_f*(-1),corner_E,corner_W,corner_N,
     1  corner_S,gdnf_x+1,gdnf_y+1," 2.5 0 200 0 0" 
                write(30,550)m_tpdata6h(:,:,n0,k0)
                close(30)
            enddo
            print *, "SUCCESS writing micaps 4 data /"//Fname_3
        enddo
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!C 生成micaps 4类 插值 后 数据
     ...
     1     "ini",pre_date,"to",cur_date,"number",numberstr,"EC24HRain"
                ...
                write(*,*)mhz
                filename_4=trim(rawpath)//"/P_"//numberstr//"_"//
     1          pre_date//"-"//cur_date(5:10)//"_M4_INS."//mhz
                write(6,*)"write to file : ",filename_4
                open(30,file=filename_4,iostat=status)
                write(30,*)comment(:len_trim(comment))
                write(30,500)year,month,day,hour,0,9999
        write(30,100)gd_l,gd_l*(-1),corner_E,corner_W,corner_N,
     1  corner_S,gdnl_x+1,gdnl_y+1," 2.5 0 200 0 0" 
            do j0=1,gdnl_y+1
            write(30,550)(czh_tp(i0,j0,n0,k0),i0=1,gdnl_x+1)
            enddo
                close(30)
            enddo
            print *, "SUCCESS writing insert m 4 data "//filename_4
        enddo
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!C 创建netCDF文件，返回文件对应的ID，如果存在则覆盖ECENS_2015100912_24_6_tp.nc
     ...
     1  latitude_dim))
        call check(nf90_def_dim(ncid_out,DimNames(3),50,
     1  number_dim))
        call check(nf90_def_dim(ncid_out,DimNames(4),k1,
     1  time_dim))
        write(*,*)'longitude_dim=',longitude_dim
        write(*,*)'latitude_dim=',latitude_dim
        write(*,*)'number_dim=',number_dim
     ...
     1  number_dims,number_id))
        write(*,*)'number_id=',number_id
        call check(nf90_def_var(ncid_out,VarNames(4),NF90_INT,time_dims,
     1  time_id))
        write(*,*)'time_id=',time_id        
        call check(nf90_def_var(ncid_out,VarNames(5),NF90_FLOAT,tp_dims,
     1  tp_id))
        write(*,*)'tp_id=',tp_id
        !C    /* assign attributes */ 
     ...
     1   "by zhang weimin"))
        ...
        call check(nf90_put_var(ncid_out,number_id,npos_out))
...
!C 写grads二进制文件
        ...
        write(15)m_tpdata6h
        close(15)
        print *, "SUCCESS writing bin file ",grdfileout
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
!C 写grads二进制文件
        write(*,*)'grdfileout=',grdfileout
        write(*,*)'pathname=',pathname(:(len_trim(pathname)-3))
        grdfileout=trim(pathname(:(len_trim(pathname)-3)))//"_INS.GRD"
        write(*,*)'grdfileout=',grdfileout
